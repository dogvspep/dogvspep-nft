...
import { awaitTransactionSignatureConfirmation, CandyMachine, getCandyMachineState, mintMultipleToken, mintOneToken } from "../solana/candy-machine";
import { useWallet } from "@solana/wallet-adapter-react";
import toast from 'react-hot-toast';
import useWalletBalance from "./useWalletBalance";
import { LAMPORTS_PER_SOL } from "@solana/web3.js";
import { sleep } from "../solana/utility";
   ...
   const onMint = async () => {
      try {
         ...
         if (wallet.connected && candyMachine?.program && wallet.publicKey) {
            ...
            if (!status?.err) {
               toast.success("Congratulations! Mint succeeded! Your DogVsPep NFT should appear in your wallet soon", { duration: 10000});
            } else {
               toast.error("Mint failed! Please check your funds and try again!",  { duration: 10000});
            }
         }
      } catch (error: any) {
         let message = error.msg || "Minting failed! Please check your funds and try again!";
         if (!error.msg) {
   ...
   const onMintMultiple = async (quantity: number) => {
      try {
         ...
         if (wallet.connected && candyMachine?.program && wallet.publicKey) {
            ...
            if(totalSuccess) {
               toast.success(`Congratulations! ${totalSuccess} mints succeeded!Your DogVsPep NFT should appear in your wallet soon`, { duration: 10000, position: "top-center" })
            }

            if(totalFailure) {
               toast.error(`Some mints failed! ${totalFailure} mints failed! Check on your wallet`, { duration: 10000, position: "top-center" })
            }
         }
      } catch (error: any) {
         let message = error.msg || "Minting failed! Please check your funds and try again!";
